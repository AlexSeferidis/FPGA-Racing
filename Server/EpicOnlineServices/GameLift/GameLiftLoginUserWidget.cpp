...
void UGameLiftLoginUserWidget::OnLoginResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	if (bWasSuccessful) {
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			...
			FString Message = JsonObject->GetObjectField("status")->GetStringField("msg");

			if(GEngine)  GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Something Got Called"));
