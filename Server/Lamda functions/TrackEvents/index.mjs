...
export const handler = async (event) => {
  ...
  if (
    messageDetail.type === 'MatchmakingSucceeded' ||
    messageDetail.type === 'MatchmakingTimedOut' ||
    messageDetail.type === 'MatchmakingCancelled' ||
    messageDetail.type === 'MatchmakingFailed'
  ) {
  ...
  // Check if there are items to write before calling batchWriteItem
  if (dynamoDbRequestParams.RequestItems.MatchmakingTickets.length === 0) {
    response = {
      statusCode: 200,
      body: JSON.stringify({
  ...
  // Call batchWriteItem only if there are items to write
